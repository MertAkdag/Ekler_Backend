...
/**
 * Approve a crowdsourced course suggestion: insert the course into the catalog
 * (idempotent on code+university_domain, same contract as the suggest_course /
 * auto_approve RPCs) and mark the suggestion approved — in one transaction. The
 * admin pool is superuser so no auth.uid() is needed (the DB RPC required it);
 * reviewed_by is left null (no admin uuid in the single-env panel, like other actions).
 */
async function approveCourseSuggestion(id: string): Promise<void> {
  const client = await pool.connect()
  try {
    await client.query('begin')
    const { rows } = await client.query(
      `select code, name, university_domain, department_id, status
         from public.course_suggestions where id = $1 for update`,
      [id],
    )
    const s = rows[0]
    if (!s) throw new Error('Öneri bulunamadı.')
    if (s.status !== 'pending') throw new Error('Bu öneri zaten incelenmiş.')
    await client.query(
      `insert into public.courses (code, name, university_domain, department_id)
       ...
       on conflict (code, university_domain) do update set
         ...
         name = excluded.name`,
      [s.code, s.name, s.university_domain, s.department_id],
    )
    await client.query(
      "update public.course_suggestions set status = 'approved', reviewed_at = now() where id = $1",
      [id],
    )
    await client.query('commit')
  } catch (err) {
...
/** Öğrenci ders önerileri — onayla (kataloğa ekle) / reddet. */
export function courseSuggestionActions(): Record<string, RecordAction> {
  return {
    approveSuggestion: recordAction({
      icon: 'Check',
      guard: 'Bu ders önerisini onaylayıp kataloğa eklemek istediğine emin misin?',
      successMessage: 'Ders önerisi onaylandı ve kataloğa eklendi.',
      entityType: 'course_suggestions',
      permissionKey: 'course_suggestions.approve',
      action: 'approve',
      mutate: approveCourseSuggestion,
    }),
    rejectSuggestion: recordAction({
      icon: 'X',
      guard: 'Bu ders önerisini reddetmek istediğine emin misin?',
      successMessage: 'Ders önerisi reddedildi.',
      entityType: 'course_suggestions',
      permissionKey: 'course_suggestions.reject',
      action: 'reject',
      mutate: async (id) =>
        void (await run(
          "update public.course_suggestions set status = 'rejected', reviewed_at = now() where id = $1",
          [id],
        )),
    }),
  }
